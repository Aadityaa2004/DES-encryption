...
class des():
    ...
    def substitute(self, d_e):  # Substitute bytes using SBOX
        ...
        for i, block in enumerate(subblocks):
            row = (block[0] << 1) | block[5]  # Row is formed by the first and last bit
            col = int(''.join(map(str, block[1:5])), 2)  # Column is formed by the middle four bits

            # Select the appropriate S-Box based on the round (i)
            s_box = S_BOX[i]
            s_value = s_box[row][col]  # Get the S-Box output
            result += [int(x) for x in binvalue(s_value, 4)]  # Convert to 4-bit binary and add to result
    ...
    def compute_s_box(self, block, round):
        row = (block[0] << 1) | block[5]  # Row from first and last bits
        col = int(''.join(map(str, block[1:5])), 2)  # Column from middle bits

        s_box = S_BOX[round]  # Select the appropriate S-Box for the round
        s_value = s_box[row][col]  # Get the S-Box output
        return [int(x) for x in binvalue(s_value, 4)]  # Return as a list of bits
